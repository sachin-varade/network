...
var queryChaincode = function(peer, channelName, chaincodeName, args, fcn, username, org, cb) {
	...
	return helper.getRegisteredUsers(username, org).then((user) => {
		...
		if (response_payloads) {
			for (let i = 0; i < response_payloads.length; i++) {
				return cb(null,JSON.parse(response_payloads[i].toString('utf8')));
			}
		} else {
			logger.error('response_payloads is null');
			return cb('response_payloads is null',null);
		}
	}, (err) => {
		logger.error('Failed to send query due to error: ' + err.stack ? err.stack :
			err);
		return cb('Failed to send query due to error: ' + err.stack ? err.stack : err, null);
	}).catch((err) => {
		logger.error('Failed to end to end test with error:' + err.stack ? err.stack :
			err);
		return cb('Failed to end to end test with error:' + err.stack ? err.stack : err, null);
	});
};
var getBlockByNumber = function(peer, blockNumber, username, org, cb) {
	var target = buildTarget(peer, org);
	var channel = helper.getChannelForOrg(org);
	...
	return helper.getRegisteredUsers(username, org).then((member) => {
		...
		logger.info('Failed to get submitter "' + username + '"');
		return cb('Failed to get submitter "' + username + '". Error: ' + err.stack ? err.stack : err, null);
	}).then((response_payloads) => {
		if (response_payloads) {
			//logger.debug(response_payloads);
			logger.debug(response_payloads);
			return cb(null, response_payloads); //response_payloads.data.data[0].buffer;
		} else {
			logger.error('response_payloads is null');
			return cb('response_payloads is null', null);
		}
		...
		return cb('Failed to query with error:' + err.stack ? err.stack : err, null);
	});
};
...
var getChainInfo = function(peer, username, org, cb) {
	var target = buildTarget(peer, org);
	var channel = helper.getChannelForOrg(org);
